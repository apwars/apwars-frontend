...
const fetchFarms = async (account: string) => {
  const data = await Promise.all(
    farmsConfig.map(async (farmConfig) => {
          ...
          const [burnRate] = await multicall(burnManager, [
            {
              ...
              name: 'getBurnRate',
              params: [
                farmConfig.farmManager,
                farmConfig.isTokenOnly ? farmConfig.tokenAddresses[CHAIN_ID] : farmConfig.lpAddresses[CHAIN_ID],
                account,
                farmConfig.internalPID,
              ],
            },
          ])
      ...
      if(allocPoint.toString() !== "0" &&  totalAllocPoint.toString()) {
